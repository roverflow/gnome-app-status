...
export class TrayIconsManager extends Signals.EventEmitter {
    static initialize(indicatorTray) {
        if (!trayIconsManager)
            trayIconsManager = new TrayIconsManager(indicatorTray);
    ...
    constructor(indicatorTray) {
        super();

        if (trayIconsManager)
            throw new Error('TrayIconsManager is already constructed');

        this._indicatorTray = indicatorTray;
    ...
    _disable() {
        ...
        this._indicatorTray.getIconsByType(IndicatorStatusIcon.IndicatorStatusTrayIcon)
            .forEach(i => i.destroy());
    ...
    onTrayIconAdded(_tray, icon) {
        ...
        this._indicatorTray.addIcon(trayIcon);
    }

    onTrayIconRemoved(_tray, icon) {
        try {
            const trayIcons = this._indicatorTray
                .getIconsByType(IndicatorStatusIcon.IndicatorStatusTrayIcon);
            const [trayIcon] = trayIcons.filter(i => i.icon === icon);
            trayIcon.destroy();
        } catch (e) {
